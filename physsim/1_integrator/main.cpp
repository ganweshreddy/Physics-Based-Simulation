...
namespace physsim
{
    ...
    public:
        ...
        void advance(double elapsedTime, double totalTime, int64_t timeStep) override
        {
            ...
            // v(t) = v_0 + a*t
            mPosition[EMethod::Analytic] =
                mInitialPosition + mInitialVelocity * mTime + 0.5 * mTime * mTime * mGravity; // <- adjust
            mVelocity[EMethod::Analytic] = mInitialVelocity + mGravity * mTime;               // <- adjust
            ...
            // x' = x + dt*v
            // v' = v + dt*a
            mPosition[EMethod::ExplicitEuler] += mStepSize * mVelocity[EMethod::ExplicitEuler];
            mVelocity[EMethod::ExplicitEuler] += mStepSize * mGravity;
            ...
            // v' = v + dt*a
            // x' = x + dt*v'
            mVelocity[EMethod::SymplecticEuler] += mStepSize * mGravity;
            mPosition[EMethod::SymplecticEuler] += mStepSize * mVelocity[EMethod::SymplecticEuler];
            addSphere(mPosition[EMethod::SymplecticEuler], 0.03, Spectrum(0, 0, 1));

            if (mPosition[EMethod::Analytic][2] < -0.5)
                finished = true;
