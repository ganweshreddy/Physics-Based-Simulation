...
#pragma omp parallel for
            ...
            for (Eigen::Index i = 0; i < mPositions->getSize(); ++i)
            {
                ...
                float pi = mStiffness * (pow(mDensities->getValue(i).value() / mRho0, mExponent) - 1); // ... set correct value
            ...
            for (Eigen::Index i = 0; i < mPositions->getSize(); ++i)
            {
                ...
                if (knn->closestRadius(xi, mSupportRadius, nn) != 0)
                {
                    for (auto& n : nn)
                    {
                        ...
                        ai_p += mViscosity * massj / rhoj * (vi - vj).dot((xi - xj).normalized()) * W.gradW(xi - xj);
                    }
                }
                ...
                if (mBoundaryKNN->closestRadius(xi, mSupportRadius, nn) != 0)
                {
                    for (auto& n : nn)
                    {
                        ...
                        ai_p += -massk * (pi / (rhoi * rhoi) + pi / (rhok * rhok)) * W.gradW(xi - xk);

                        // TODO: viscosity acceleration (Akinci)
                        ai_p += mViscosity * massk / rhok * vi.dot((xi - xk).normalized()) * W.gradW(xi - xk);
                    }
                }
